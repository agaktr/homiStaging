...
class HomiRegister {
    ...
    const AJAX_CHECK_EMAIL = 'ajax_check_email';

    const REGISTER_PAGE_SLUG = 'register';
    ...
    /**
     * Force login of the new user with ID: $user_id
     * and redirect to pricing page, unless the new user came from the payment page
     *
     * @param $user_id
     *
     * @return void
     */
    public function redirect_and_login_new_user( $user_id ){
    ...
    public function changeRegisterMessages( $translated_text ){

        if($translated_text === 'Sorry, that username already exists!'){
            $translated_text = 'Sorry, this username is already taken.';
        }
    ...
    public function registerNewUser() {
        ...
        if( isset( $_POST['new_user'] ) ) {
            ...
            $user_args['role']       = Homi_Addons::BUYER_ROLE;

            $user_id = wp_insert_user( $user_args );

            if ( ! $user_id instanceof WP_Error ) {

                $value = $_POST['fave_author_mobile'];

                if ( null !== $value ) {

                    add_user_meta( $user_id, 'fave_author_mobile', $value );

                }

                setcookie('new_user_message', true, time() + (86400 * 30), "/");

                $this->redirect_and_login_new_user( $user_id );

            }
            else {

                wp_redirect( site_url('register') . '?new_user=fail&error_message=' . sanitize_title_for_query( $user_id->get_error_message() )  );
    ...
    public function successful_registration_message(){
        ...
        if( isset( $_COOKIE['new_user_message'] ) ){
            ...
            <div class="register-overlay">
                ...
                <div id="registerModal" class="register-success">
                    ...
                    <p>
                        You can now enjoy all the features of the Viewer account.
                    </p>
    ...
    public function custom_register_login_buttons(){
        ?>

        <a id="loginBtn" href="<?php echo wp_login_url(); ?>" class="homi-btn">
            Sign in
        </a>

        <a id="registerBtn" href="<?php echo wp_registration_url(); ?>" class="homi-btn">
            Register
        </a>
    ...
    public function ajax_check_email(){
        ...
        $result['email']        = $email_to_check;
        $result['available']    = isset( $_POST['email'] ) && email_exists( $email_to_check ) === false;
