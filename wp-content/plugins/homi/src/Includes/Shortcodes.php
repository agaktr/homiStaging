 ...
 *
 * @package    Homi
 * @subpackage Homi/Includes
 */

namespace Homi\Includes;

use Homi\Traits\Core\ClassList;
...
class Shortcodes {
	 ...
	 *
	 * @see \Homi\Traits\Core\ClassList
	 */
	use ClassList;
	 ...
	 * and if it implements the @see \Homi\Interfaces\ShortcodesInterface
	 *
	 */
		...
		if( is_array( $this->classes ) && !empty( $this->classes ) ){
			...
			foreach( $this->classes as $shortcode ){
				...
				if( class_exists( $shortcode ) ){
					...
					if ( isset( $interfaces['Homi\Interfaces\ShortcodesInterface'] ) ) {

						/**
						 * @see \Homi\Shortcodes\ShortcodeBoilerplate
						 * @see \Homi\Interfaces\ShortcodesInterface
						 */
