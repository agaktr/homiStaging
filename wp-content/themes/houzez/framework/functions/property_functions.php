    ...
    function houzez_property_search_2($search_query)
    {
        ...
        if (isset($_GET['keyword']) && $_GET['keyword'] != '') {
            if ($keyword_field == 'prop_address') {
                $meta_keywork = sanitize_text_field(stripcslashes($_GET['keyword']));
                $address_array = array(
                ...
                foreach ( $fields_array as $key => $value ):
                    ...
                    if( $is_search == 'yes' ) {
                        if(!empty($dummy_array[$key])) {
                            $meta_query[] = array(
                                'key' => 'fave_'.$field_name,
                                'value' => sanitize_text_field($dummy_array[$key]),
                                'type' => 'CHAR',
                                'compare' => '=',
                            );
                        }
...
if( !function_exists('houzez_half_map_listings') ) {
    function houzez_half_map_listings() {
        ...
        if( !empty( $features ) ) {
            foreach ($features as $feature):
                $features = sanitize_text_field($features);
                $tax_query[] = array(
                    'taxonomy' => 'property_feature',
                    'field' => 'slug',
                    'terms' => $feature
        ...
        if ( !empty( $labels ) ) {
            $labels = sanitize_text_field($labels);
            $tax_query[] = array(
                'taxonomy' => 'property_label',
                'field' => 'slug',
                'terms' => $labels
                        ...
                        <div class="print-block">
                            ...
                            <div class="alert">
                                <ul class="print-list-three-col">
                                    ...
                                    if( !empty( $land_area ) ) {
                                        echo '<li><strong>'.esc_html__( 'Land Area:', 'houzez'). '</strong> '.houzez_property_land_area_by_id( $property_id, 'after' ).'</li>';
                                    }
                                    if( !empty( $bedrooms ) ) {
       ...
       $agent_phone_full = houzez_option('agent_phone_full');
        ...
        if( $type == 'for_grid_list' ) {
            ...
            if( !empty( $args[ 'facebook' ] ) ) :
                ...
                $output .= '<span><a class="btn-facebook" target="_blank" href="' . esc_url($args[ 'facebook' ]) . '"><i class="fa fa-facebook-square"></i> <span>Facebook</span></a></span>';

            endif;

            if( !empty( $args[ 'instagram' ] ) ) :

                $output .= '<span><a class="btn-instagram" target="_blank" href="' . esc_url($args[ 'instagram' ]) . '"><i class="fa fa-instagram"></i> <span>Instagram</span></a></span>';

            endif;

            if( !empty( $args[ 'twitter' ] ) ) :

                $output .= '<span><a class="btn-twitter" target="_blank" href="' . esc_url($args[ 'twitter' ]) . '"><i class="fa fa-twitter-square"></i> <span>Twitter</span></a></span>';

            endif;

            if( !empty( $args[ 'linkedin' ] ) ) :

                $output .= '<span><a class="btn-linkedin" target="_blank" href="' . esc_url($args[ 'linkedin' ]) . '"><i class="fa fa-linkedin-square"></i> <span>Linkedin</span></a></span>';

            endif;

            if( !empty( $args[ 'googleplus' ] ) ) :

                $output .= '<span><a class="btn-googleplus" target="_blank" href="' . esc_url($args[ 'googleplus' ]) . '"><i class="fa fa-google-plus-square"></i> <span>Google Plus</span></a></span>';

            endif;

            if( !empty( $args[ 'youtube' ] ) ) :

                $output .= '<span><a class="btn-youtube" target="_blank" href="' . esc_url($args[ 'youtube' ]) . '"><i class="fa fa-youtube-square"></i> <span>Youtube</span></a></span>';
